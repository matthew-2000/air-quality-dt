...
def apply_scenario(
    ...
    def scenario_value(row: pd.Series) -> float:
        ...
        traffic_delta = -traffic_coeff * float(row.get("traffic_index", 0.0)) * traffic_reduction
        ...
        return max(0.0, float(row["estimated_value"]) + traffic_delta + wind_delta + rain_delta)
